...
public class AvroUtils {
    ...
    private static Schema.Field toField(ColumnMetadata column) {
        final String name = StringUtils.isEmpty(column.getName()) ? DATAPREP_FIELD_PREFIX + column.getId()
    ...
    private static Optional<ColumnMetadata> getColumnMetadata(Schema.Field field) {
        ...
        return Optional.of(ColumnMetadata.Builder
                .column() //
                .type(Type.get(field.schema().getType().getName())) //
    ...
    public static DataSetRow toDataSetRow(IndexedRecord indexedRecord, AvroUtils.Metadata metadata) {
        if (metadata == null) {
            throw new IllegalArgumentException("Metadata cannot be null.");
        }
        final List<Schema.Field> fields = indexedRecord.getSchema().getFields();

        final List<ColumnMetadata> newColumns = metadata.columns.stream().map(c -> ColumnMetadata.Builder.column().copy(c).build()).collect(Collectors.toList());
        ...
        return row.setTdpId(metadata.getRowId());
    }
